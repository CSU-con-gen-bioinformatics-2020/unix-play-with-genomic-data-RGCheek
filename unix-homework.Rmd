...
is a consequence of RMarkdown/html not playing well with the shell's way of
...
Are all hidden files. Though I'm not sure what the "." and ".." files are, unless they are just directions to the current directory or the one above it. 
...
```
I did it in 11 key strokes. I hit tab a few more times since there are multiple files that start with DPCh_plate1_A.
```
...
mkdir -p lonely_fastqs/R1

mkdir lonely_fastqs/R2
...
cp fastq/*.R1.fq.gz lonely_fastqs/R1

cp fastq/*.R2.fq.gz lonely_fastqs/R2
...
echo 'Woah! No spaces, please!' > ./bad\ directory\ name\ with\ spaces/bad\ file\ name\ with\ spaces.txt
...
```
You want to list the directories you plan on deleting to make sure you don't accidently delete something important. By listing them first you can see what the computer thinks you're talking about first. 
```
...
```
I don't think Unix would have been able to remove any files in the current directory since it would try to delete directories that are called, "bad", "directory", "name", "with", or "spaces". Plus, when I try 'ls bad directory name with spaces', I get this error:
...
#use the 2> to output the sterror on the #2 stream
...
```
The names in DPCh_plate1_C11_S35.R2.fq.gz look the same compared to DPCh_plate1_C11_S35.R1.fq.gz, except one ends with a 2, and the other ends with a 1. 
```
...
head sam/DPCH_plate1_A05_S5.sam | wc

head sam/DPCH_plate1_A06_S6.sam | wc
...
```{bash}
#write to text file 
 zcat fastq/*R1.fq.gz > R1_files.txt
 
 wc -l fastq/R1_files.txt
...
zcat fastq/*R1.fq.gz > fastq/R1_all_via_gzcat.fq
...
zcat fastq/R1_all_via_cat.fq.gz > fastq/R1_all_via_cat.fq
...
sha1sum fastq/R1_all_via_gzcat.fq
...
gzip fastq/R1_all_via_gzcat.fq
...
```
This action of compressing files reduces teh size by a factor of ~4. 
```
...
runas chmod -R 664 ./count_files.sh
...
**I'm not sure why chmod isn't working. I thought it had something to do with Dropbox, but I moved the project into a directory on my desktop and the problem continues.** 
...
#currently I have read and write permissions as a user.
#to make it executable for users and groups
...
#heres what I think the codes should be to get the desired outcomes. Chmod still won't change the permissions, and apparently this is a reoccuring issue on Stack exchange but I haven't been able to find a solution that works. supposedly runas should work for windows, but not for me.

chmod 0774 ./silly/FILE-rwxrwxr--

chmod 0740 ./silly/FILE-rwxr-----

chmod 0664 ./silly/FILE-rw-rw-r--

chmod 0664 ./silly/FILE-rw-rw-r--

chmod 0771 ./silly/DIRdrwxrwxr-x/
